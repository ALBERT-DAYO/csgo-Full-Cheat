...
bool CBone::UpdateAllBoneData(const DWORD64& EntityPawnAddress)
{
	...
	if (!m.ReadMemory<DWORD64>(EntityPawnAddress + Offset::Pawn.GameSceneNode, GameSceneNode))
		return false;
	if (!m.ReadMemory<DWORD64>(GameSceneNode + Offset::Pawn.BoneArray, BoneArrayAddress))
		return false;

	BoneJointData BoneArray[30]{};
	if (!m.ReadMemory(BoneArrayAddress, BoneArray, 30 * sizeof(BoneJointData)))
		return false;
